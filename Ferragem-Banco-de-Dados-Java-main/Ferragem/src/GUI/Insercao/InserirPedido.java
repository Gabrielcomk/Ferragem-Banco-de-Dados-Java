...
private int idCliente;
 ...
 int row = tabelaEstoque.getSelectedRow();
    ...
    private void FinalizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_FinalizarActionPerformed
          try {
        // 1 - Criar nota fiscal para o cliente
        DAO.notaFiscalDAO nfDao = new DAO.notaFiscalDAO();
        int nrNF = nfDao.criarNotaFiscal(idCliente);

        // 2 - Inserir todos os itens da tabela de escolhidos
        DAO.PedidoDAO pDao = new DAO.PedidoDAO();
        DefaultTableModel modelo = (DefaultTableModel) tabelaItensEscolhidos.getModel();

        for (int i = 0; i < modelo.getRowCount(); i++) {
            int codigoItem = Integer.parseInt(modelo.getValueAt(i, 0).toString());
            int qtd = Integer.parseInt(modelo.getValueAt(i, 5).toString());

            Classes.Pedido p = new Classes.Pedido(codigoItem, nrNF, qtd,
