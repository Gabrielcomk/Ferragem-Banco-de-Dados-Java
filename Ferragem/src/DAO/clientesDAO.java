...
public class clientesDAO {
    ...
    public void Inserir(Clientes cliente) throws ErpDAOException {
        ...
        try {
            ...
            throw new ErpDAOException("Erro ao inserir um novo " + sqle);
        } finally {
            Conexao.close(connL, ps);
        }
    }
   ...
   public void InserirComDependencias(Clientes cliente) throws ErpDAOException {
    PreparedStatement ps = null;
    Connection connL = null;
    ...
    try {
        ...
        connL.setAutoCommit(false); // inicia transação
        ...
        connL.commit(); // confirma a transação

    } catch (SQLException e) {
        try {
            if (connL != null) connL.rollback(); // desfaz a transação em caso de erro
        } catch (SQLException ex) {
            throw new ErpDAOException("Erro no rollback: " + ex.getMessage(), ex);
        }
        throw new ErpDAOException("Erro ao inserir cliente com dependências: " + e.getMessage(), e);
    } finally {
        Conexao.close(connL, ps);
    }
}
