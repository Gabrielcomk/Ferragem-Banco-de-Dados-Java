...
public class clientesDAO {
    ...
    String SQL = "SELECT p.codigo_item, p.NR_NF, p.quantidade, e.item, e.preco, " +
                 "       (p.quantidade * e.preco) AS valor_total " +
                 "FROM pedido p " +
        ...
        try (ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                Pedido pedido = new Pedido(
                    ...
                    rs.getString("item"),
                    rs.getDouble("preco"),        // <<-- novo campo
                    rs.getDouble("valor_total")   // <<-- total calculado
                );
                pedidos.add(pedido);
            }
        }
